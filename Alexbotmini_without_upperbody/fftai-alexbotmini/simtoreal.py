...
class utils:
    ...
    def run(policy):
        global pos, vel, server_ip_list
        target_q = np.zeros((env.num_actions), dtype=np.double)
        ...
        if count_lowlevel % 10 == 0:
            ...
            q = np.array(pos)
            dq = np.array(vel)
            obs = np.zeros([1, env.num_single_obs], dtype=np.float32)
            eu_ang = utils.quaternion_to_euler_array(normalization.obs_scales.quat)
            eu_ang[eu_ang > math.pi] -= 2 * math.pi

            # Get obs
            obs[0, 0] = math.sin(2 * math.pi * count_lowlevel * normalization.dt  / 0.64)
            obs[0, 1] = math.cos(2 * math.pi * count_lowlevel * normalization.dt  / 0.64)
            obs[0, 2] = cmd.vx * normalization.obs_scales.lin_vel
            obs[0, 3] = cmd.vy * normalization.obs_scales.lin_vel
            obs[0, 4] = cmd.dyaw * normalization.obs_scales.ang_vel
            obs[0, 5:17] = q * normalization.obs_scales.dof_pos
            obs[0, 17:29] = dq * normalization.obs_scales.dof_vel
            obs[0, 29:41] = action
            #obs[0, 41:44] = omega
            #obs[0, 44:47] = eu_ang
            ...
            policy_input = np.zeros([1, env.num_observations], dtype=np.float32)
            for i in range(env.frame_stack):
                policy_input[0, i * env.num_single_obs : (i + 1) * env.num_single_obs] = hist_obs[i][0, :]
            action[:] = policy(torch.tensor(policy_input))[0].detach().numpy()
            action = np.clip(action, -normalization.clip_actions, normalization.clip_actions)
            target_q = action * normalization.action_scale
